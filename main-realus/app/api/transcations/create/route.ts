...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Get the logged-in user using the new helper
    
    let agentId: any = "test-agent-id";
    let brokerId = "test-broker-id";
    let agentName = "Unknown Agent";

    if (!user) {
      console.log("No authenticated user found - using test IDs");
      ...
      const agent = await User.findById(user.id) as UserType | null;
